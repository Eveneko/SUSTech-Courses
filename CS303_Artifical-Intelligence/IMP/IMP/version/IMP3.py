...
class Worker(mp.Process):
    ...
    def run (self):
        while True:
            ...
            theta = task
            if self.model == 'IC':
                R = IC_rr(self.network, theta)
            elif self.model == 'LT':
                R = LT_rr(self.network, theta)
            self.outQ.put(R)        


def create_worker(num, model, network):
    global worker
    for i in range(num):
...
def Sampling(network, k, e, l):
    ...
    while len(R) <= theta:
        if model == 'IC':
            R.append(IC_rr(network, 1)[0])
        elif model == 'LT':
            R.append(LT_rr(network, 1)[0])
    return R
...
if __name__ == '__main__':
    ...
    worker = []
    worker_num = 8
    create_worker(worker_num, model, network)

    l = 1
    if network.vertex_cnt < 10000:
        if time_budget >= 120:
            e = 0.1
        if k >= 50:
            e = 0.2
            ...
            e = 0.1
    elif 10000 <= network.vertex_cnt < 30000:
        if time_budget >= 120:
            e = 0.1
        if k >= 50:
            e = 0.3
    ...
    # ISE_test(network_file_path, S_k_star, model, 60)
