...
//				}
				...
				int presmall = 0;
				int sumweight = 0;
				for (start = 0; start < edges.length; start++) {
					int smallestEdgeWeight = edges[start].w;
					if (presmall != smallestEdgeWeight) {
						int tmpsumweight = kruskal(edges, parent, n, m, start);
						if (tmpsumweight == -1) {
							break;
						}
						sumweight = tmpsumweight;
					}
					presmall = smallestEdgeWeight;
				}
				out.println(sumweight);
			}
		}

		public int kruskal(edge[] edges, int[] parent, int n, int m, int start) {
			int sumweight = 0;
			int num = 0;// 已选用的边的数目
			int u, v;
			for (int i = 0; i < parent.length; i++) {
